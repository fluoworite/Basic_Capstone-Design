...
#define FLAME_SENSOR_PIN 2  // 화염 센서 핀
#define LED_PIN 4 
...
<body>
    <div class="container">
        ...
        <div class="controls">
            <div>
                <button class="button" onmousedown="toggleCheckbox('speed100');"
                    ontouchstart="toggleCheckbox('speed100');">Speed</button>
            </div>
        </div>
        <table>
            <tr>
...
void setup() {
  ...
  pinMode(FLAME_SENSOR_PIN, INPUT);   // 화염 센서 핀을 입력으로 설정
...
void loop() {
  int flameDetected = digitalRead(FLAME_SENSOR_PIN);  // 화염 센서 상태 읽기
  if (flameDetected == LOW) {  // 화염이 감지되면
  car_speed = map(0,0,100,0,255);
    Serial.println("Flame detected!");
    digitalWrite(LED_PIN, HIGH);  // LED 켜기
    delay(500);                  // 0.5초 동안 켜진 상태 유지
    digitalWrite(LED_PIN, LOW);   // LED 끄기
    delay(500);                  // 0.5초 동안 꺼진 상태 유지
  } else {
    Serial.println("No flame detected.");
    digitalWrite(LED_PIN, LOW);   // 화염이 감지되지 않으면 LED 끄기
  }
  delay(100);  // 다음 읽기까지 100밀리초 대기
}
